...
/// <summary>
/// Renders all creatures and the local player, Y-sorted (painter's algorithm).
///
/// Per entity:
///   - Tile target square  (targeted creature only, yellow)
///   - Sprite + border
///   - HP bar above sprite (creatures AND local player)
///   - Name tag above HP bar (creatures only)
/// </summary>
public class CreatureRenderSystem : ISystem
{
    ...
    public void Update(float deltaTime)
    {
             ...
             ref CreatureRenderOrder order) =>
        {
            ...
            if (isCreature && entity.Has<CreatureHpComponent>())
            {
                hpPct     = entity.Get<CreatureHpComponent>().HpPct;
                showHpBar = true;
            }
            else if (isLocal && entity.Has<StatsDataComponent>())
            {
                ref var stats = ref entity.Get<StatsDataComponent>();
                hpPct = stats.MaxHP > 0
                    ? (byte)Math.Clamp(stats.CurrentHP * 100 / stats.MaxHP, 0, 100)
                    : (byte)0;
                showHpBar = true;
            }
            ...
            _drawCalls.Add((sortY, () =>
            {
                ...
                if (cShowHpBar)
                {
                    ...
                    var bgCol   = cIsLocal ? new Color(0, 40, 0, 200) : new Color(60, 0, 0, 200);
                    var fillCol = cIsLocal
                        ? new Color(30, 200, 30, 255)
                        : cHpPct > 50 ? new Color(0,  200,  0, 255)
                    ...
                    Raylib.DrawRectangleLines(barX, barY, barW, barH, new Color(80, 80, 80, 180));

                    // ── Name tag above bar (creatures only) ───────────────
                    if (cIsCreature && !string.IsNullOrEmpty(cName))
                    {
